<template>
  ...
  >
    <VCard v-if="proyecto" title="Asignar Proyecto">
      <VCardText>
        <p class="mb-4">
          Se asignará el proyecto a la EECC: <strong class="text-primary">{{ proyecto.eecc }}</strong>
        </p>
...
const nuevaIp = ref('')
const ipInputRef = ref(null) // Referencia para el campo de IP

// Reseteamos la nueva IP cada vez que se abre el diálogo
watch(() => props.open, async (isOpen) => {
  if (isOpen) {
    nuevaIp.value = ''
    // Si el campo de IP es visible, le damos foco

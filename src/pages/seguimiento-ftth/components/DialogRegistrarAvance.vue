  ...
  >
    <VCard title="Registrar Avance">
      ...
      <VCardText>
        ...
        <VForm
          v-for="(trabajo, index) in formState.trabajos"
          :key="trabajo.id"
        ...
        >
          <VRow align="center">
            ...
            <VCol cols="12" sm="3">
              <VTextField
                ...
                :min="0"
                :readonly="
                  Number(trabajo.trabajados_original) === Number(trabajo.total)
                "
                :rules="[
              ...
              >
                <div class="d-flex justify-space-between mb-1">
                  ...
                  <span class="text-caption font-weight-bold"
                    >{{ calcularAvance(trabajo).toFixed(2) }}%</span
                  >
                </div>
                <VProgressLinear
            ...
            <VCol cols="12" sm="3" class="text-center">
              <VBtn
                color="primary"
                @click="actualizarTrabajo(trabajo, index)"
          ...
          <VRow
            v-if="trabajo.nombre === 'INSTALACION PASIVOS' && formState.mufas"
          >
            <VCol cols="12" sm="4">
              <VTextField
      ...
      <VCardActions>
        <VSpacer />
        <VBtn color="secondary" @click="dialogValueUpdate(false)">
          Cerrar
        </VBtn>
      </VCardActions>
    </VCard>
  ...
  (newData) => {
    ...
    if (dataCopy.trabajos && Array.isArray(dataCopy.trabajos)) {
      dataCopy.trabajos.forEach((t) => {
        ...
        t.trabajados_original =
          t.trabajados === null ? 0 : Number(t.trabajados);
    ...
    if (
      dataCopy.mufas &&
      Array.isArray(dataCopy.mufas) &&
      dataCopy.mufas.length > 0
    ) {
      const mufasData = dataCopy.mufas[0];
      mufasData.mufas_instaladas =
        mufasData.mufas_instaladas === null
          ? ""
          : Number(mufasData.mufas_instaladas);
      mufasData.mufas_empalmados =
        mufasData.mufas_empalmados === null
          ? ""
          : Number(mufasData.mufas_empalmados);
      mufasData.rotulado =
        mufasData.rotulado === null ? "" : Number(mufasData.rotulado);
...
watch(
  ...
  (newMufas, oldMufas) => {
    ...
    if (props.isDialogVisible && newMufas) {
      ...
      const pasivosIndex = formState.value.trabajos.findIndex(
        (t) => t.nombre === "INSTALACION PASIVOS"
      );
  ...
  try {
    if (trabajo.nombre === "INSTALACION PASIVOS") {
      const response = await $api(
        `/internodal/trabajos/actualizar-trabajo-mufas/${trabajo.id}`,
        {
          method: "PUT",
          body: payload,
      ...
      const response = await $api(
        `/internodal/trabajos/actualizar-trabajo-avance/${trabajo.id}`,
        {
          method: "PUT",
          body: payload,
          onResponseError({ response }) {},
    ...
    emit("trabajoActualizado", {
      message: `'${trabajo.nombre}' actualizado correctamente.`,
      color: "secundary",
    });
  } catch (error) {
    console.error("Error al actualizar el trabajo:", error);
    emit("trabajoActualizado", {
      message: `Error al actualizar '${trabajo.nombre}'.`,
      color: "error",
    });
  } finally {
...
const reglaValorCambiado = (trabajo) => (valor) => {
  const trabajoPrincipalCambiado =
    (Number(trabajo.trabajados) || 0) !==
    (Number(trabajo.trabajados_original) || 0);

  if (trabajo.nombre === "INSTALACION PASIVOS" && formState.value.mufas) {
    const mufas = formState.value.mufas;
    const mufasOriginal = props.detalleData.mufas[0] || {};

    const instaladasCambiado =
      (Number(mufas.mufas_instaladas) || 0) !==
      (Number(mufasOriginal.mufas_instaladas) || 0);
    const empalmadasCambiado =
      (Number(mufas.mufas_empalmados) || 0) !==
      (Number(mufasOriginal.mufas_empalmados) || 0);
    const rotuladoCambiado =
      (Number(mufas.rotulado) || 0) !== (Number(mufasOriginal.rotulado) || 0);

    // Si alguno de los campos (principal o de mufas) ha cambiado, la regla es válida.
    if (
      trabajoPrincipalCambiado ||
      instaladasCambiado ||
      empalmadasCambiado ||
      rotuladoCambiado
    ) {
...
const reglaNoMenor = (trabajo) => (valor) => {
  ...
  return (
    ...
    `El valor no puede ser menor que ${valorOriginal}.`
  );
};
...
const reglaNoMayorQueTotal = (trabajo) => (valor) => {
  ...
  return (
    valorActual <= total || `El valor no puede exceder el total de ${total}.`
  );
};
...
const reglaEmpalmadosNoMayorQueInstaladas = (valor) => {
  ...
  return (
    empalmadas <= instaladas || "No puede haber más empalmadas que instaladas."
  );
};
...
const reglaRotuladoNoMayorQueInstaladas = (valor) => {
  ...
  return (
    rotuladas <= instaladas || "No puede haber más rotuladas que instaladas."
  );
};
...
const reglaMufaNoMenorQueOriginal = (mufaFieldKey) => (valor) => {
  ...
  return (
    valorActual >= valorOriginal ||
    `No puede ser menor que el valor original (${valorOriginal}).`
  );
};
</script>

...
const parseTime = (str) => {
  ...
  return (
    parseInt(parts[1]) * 24 * 60 + parseInt(parts[2]) * 60 + parseInt(parts[3])
  );
};
  ...
  async function fetchEECCs() {
    try {
      ...
      const response = await $api(
        `internodal/reporte/reporte-avance-tramos?all=true&perPage=1000`,
        {
          method: "GET",
        }
      );
      ...
      if (response.success) {
        ...
        const despliegues = new Set(
          allItems.map((item) => item.desplieges_router)
        );
    ...
    try {
      ...
      if (response.success) {
        ...
        allItems.forEach((item) => {
          ...
          distribucionDespliegues[valor] =
            (distribucionDespliegues[valor] || 0) + 1;
        });
  ...
  async function listar_asignaciones() {
    ...
    try {
      ...
      if (selectedStatus.value)
        params.append(
          "status",
          selectedStatus.value === "Asignados" ? "asignado" : "sin_asignar"
        );
      if (selectedUpdateTime.value)
        params.append(
          "updateTime",
          selectedUpdateTime.value === "< 24h" ? "lt24h" : "gt24h"
        );
      if (selectedDespliegue.value)
        params.append("desplieges_router", selectedDespliegue.value);
      ...
      const response = await $api(
        `internodal/reporte/reporte-avance-tramos?${params.toString()}`,
        {
          method: "GET",
        }
      );
      ...
      if (response.success && response.result) {
        ...
        if (selectedDespliegue.value) {
          ...
          const desplieguesEnResultados = new Set(
            itemsList.value.map((item) => item.desplieges_router)
          );
        }
